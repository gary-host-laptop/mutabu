...
(function() {
    ...
    const resetBtn  = document.getElementById('timer-trash');
...
(async function applySettings() {
    ...
    const activeIdx = (profImages && profImages.length > 0)
        ? Math.floor(Math.random() * profImages.length) : -1;
    ...
    const headerWrap = document.getElementById('header-profile-img-wrap');
    if (headerWrap && activeIdx >= 0) {
        const hImg = document.createElement('img');
        hImg.src = profImages[activeIdx].dataUrl;
        hImg.alt = '';
        hImg.classList.add('active');
        headerWrap.appendChild(hImg);
    }
