...
(function () {
  ...
  function startCycle() {
    ...
    interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        startBtn.querySelector("i").className = "ph-light ph-arrow-clockwise";
      }
    }, 1000);
  }
  ...
  trashBtn.addEventListener("click", reset);
})();
...
(function () {
  ...
  Object.keys(TRACKS).forEach((k) => {
    state[k] = { buffer: null, gainNode: null, source: null };
  });
  ...
  async function initAudio() {
    ...
    await Promise.all(
      Object.entries(TRACKS).map(async ([key, t]) => {
        try {
  ...
  Object.entries(TRACKS).forEach(([key, t]) => {
    document.getElementById(t.id).addEventListener("input", (e) => {
      if (state[key].gainNode)
        state[key].gainNode.gain.value = parseFloat(e.target.value);
    });
  });
})();

/* ── RAIN SLIDER HIT AREA EXPANSION ─────────────────────────── */
function expandSliderHitArea(slider, extraPx = 14) {
  ...
  overlay.style.cssText =
    ...
    "px; bottom:-" +
    extraPx +
    "px; left:0; right:0; cursor:pointer; z-index:1;";
  ...
  function setFromPointer(e) {
    ...
    slider.value = Math.round((min + ratio * (max - min)) / step) * step;
...
document
  .querySelectorAll(".rain-slider")
  .forEach((s) => expandSliderHitArea(s, 14));
...
document.getElementById("clock").addEventListener("click", () => {
  ...
  Store.set("nt_clock_binary", binaryClock);
  localStorage.setItem("nt_clock_binary", binaryClock ? "1" : "");
  const el = document.getElementById("clock");
...
// Restore binary clock synchronously before first tick to avoid flash
if (localStorage.getItem("nt_clock_binary") === "1") {
  binaryClock = true;
  const _el = document.getElementById("clock");
  _el.classList.add("binary");
}
